...
const Hero = () => {
  ...
  // Animation effect for the floating button
  useEffect(() => {
    const animateButton = () => {
      const time = new Date();
      const value = Math.sin(time.getTime() * 0.001) * 6; // Controls floating amplitude
      setYPosition(value);
    };

    const animationFrame = setInterval(animateButton, 20);
  ...
  return (
    <>
      ...
      >
        {/* Left Section */}
        <div className="left-section pt-10" role="region" aria-label="Hero Text">
        ...
        {/* Right Section - Aesthetic Graphics */}
        <div
        ...
        >
              ...
              <div className="w-64 h-64 bg-white rounded-full shadow-lg flex items-center justify-center">
                <div className="w-52 h-52 bg-[#5FA2FF] bg-opacity-10 rounded-full flex items-center justify-center">
                  <div className="relative">
                    ...
                    {/* Heart Beat Line */}
                    <Activity size={100} className="text-[#5FA2FF]" strokeWidth={1.5} />
                  </div>
                </div>
              </div>
          ...
          {/* Floating CTA Button */}
          <div 
            className="absolute bottom-0 mb-6 transition-all duration-100 ease-in-out"
